...
export default function App() {
    ...
    // V2 CONTROLLERS FOR DYNAMIC SHARING SUB-TYPES
    const [shareTargetType, setShareTargetType] = useState('employees'); 
...
const [editingProjectId, setEditingProjectId] = useState(null);
    ...
    const loadDashboardData = async () => {
        ...
        try {
            if (activeTab === 'dashboard') {
                ...
                if (rRes && rRes.data) {
                    setRenewals(Array.isArray(rRes.data.data) ? rRes.data.data : (Array.isArray(rRes.data) ? rRes.data : []));
                }
            } else if (activeTab === 'employees') {
                const empRes = await employeesAPI.getAll();
                if (empRes && empRes.data) {
                    setProfiles(Array.isArray(empRes.data.data) ? empRes.data.data : (Array.isArray(empRes.data) ? empRes.data : []));
                }
            } else if (activeTab === 'clients') {
                const clRes = await clientsAPI.getAll();
                if (clRes && clRes.data) {
                    setProfiles(Array.isArray(clRes.data.data) ? clRes.data.data : (Array.isArray(clRes.data) ? clRes.data : []));
                }
                const pRes = await projectAPI.getAll();
                if (pRes && pRes.data) {
                    setProjects(Array.isArray(pRes.data.data) ? pRes.data.data : (Array.isArray(pRes.data) ? pRes.data : []));
    ...
    const handleOpenSharingPanel = async (targetProject, event) => {
        ...
        try {
            ...
            setModalEmployees(empRes && empRes.data ? (Array.isArray(empRes.data.data) ? empRes.data.data : (Array.isArray(empRes.data) ? empRes.data : [])) : []);
            setModalClients(clRes && clRes.data ? (Array.isArray(clRes.data.data) ? clRes.data.data : (Array.isArray(clRes.data) ? clRes.data : [])) : []);
            setProjectShares(sActive && sActive.data ? (Array.isArray(sActive.data.data) ? sActive.data.data : (Array.isArray(sActive.data) ? sActive.data : [])) : []);
    ...
    const selectProjectWorkspace = async (project) => {
        ...
        try {
            ...
            setCredentials(cRes && cRes.data ? (Array.isArray(cRes.data.data) ? cRes.data.data : (Array.isArray(cRes.data) ? cRes.data : [])) : []);
            setNotes(nRes && nRes.data ? (Array.isArray(nRes.data.data) ? nRes.data.data : (Array.isArray(nRes.data) ? nRes.data : [])) : []);
            setReminders(rRes && rRes.data ? (Array.isArray(rRes.data.data) ? rRes.data.data : (Array.isArray(rRes.data) ? rRes.data : [])) : []);
            setFiles(fRes && fRes.data ? (Array.isArray(fRes.data.data) ? fRes.data.data : (Array.isArray(fRes.data) ? fRes.data : [])) : []);
    ...
    const handleVerifyDecrypt = async (e) => {
        ...
        try {
            ...
            setDecryptedPasswords(prev => ({
                ...prev,
                [targetId]: res.data?.password || res.data
            }));
            ...
            <div className="flex flex-1 flex-col md:flex-row overflow-hidden">
                ...
                <aside className="w-full md:w-64 bg-slate-900/20 p-4 border-r border-slate-800/80 space-y-1">
                    <button 
                        ...
                        className={`w-full flex items-center space-x-3 px-4 py-2.5 rounded-lg text-xs font-semibold tracking-wide transition ${activeTab === 'dashboard' ? 'bg-violet-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-900'}`}
                    >
                ...
                <main className="flex-1 p-6 overflow-y-auto">
                    ...
                    {!loading && activeTab === 'employees' && (
                        <div className="max-w-4xl mx-auto bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-4 shadow-xl">
                            <div>
                                ...
                                <p className="text-xs text-slate-400">Map internal developer roles across infrastructure profiles.</p>
                            </div>
                            <div className="divide-y divide-slate-800/60 text-xs font-mono">
                                {!Array.isArray(profiles) || profiles.length === 0 ? (
                                    <div className="text-center py-6 text-slate-500">No employee profiles registered.</div>
                                ) : (
                                    profiles.map(p => (
                    ...
                    {/* CLIENT PORTAL ACCOUNT ASSIGNMENT VIEW */}
                    {!loading && activeTab === 'clients' && (
                        <div className="max-w-4xl mx-auto bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-4 shadow-xl">
                            <div>
                                <h2 className="text-md font-bold text-white">External Customer Portal Assignments</h2>
                                <p className="text-xs text-slate-400">Link tenant client rows to an isolated project space.</p>
                            </div>
                            <div className="divide-y divide-slate-800/60 text-xs font-mono">
                                {!Array.isArray(profiles) || profiles.length === 0 ? (
                                    <div className="text-center py-6 text-slate-500">No client profile nodes mapped.</div>
                                ) : (
                                    profiles.map(p => (
                                        <div key={p.id} className="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800/40">
                                            <div>
                                                ...
                                                <span className="text-[11px] text-slate-500 block mt-0.5">Linked Workspace: <span className="text-violet-400 font-bold font-mono">{p.projects?.name || 'None'}</span></span>
                                            </div>
                                            <select 
                    ...
                    {/* ⚡ HERE IS THE SPECIFIC POSITION WHERE THE DASHBOARD LOGIC SNIPPET LIVES NOW: */}
                    {!loading && activeTab === 'dashboard' && (
                        <>
                            {/* If NO project is selected, show the overall pipelines list grid */}
                            {!selectedProject ? (
        ...
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
            ...
            >
                <Trash2 className="w-3.5 h-3.5" />
                                ...
                                /* If a project IS selected, keep the deep workspace panel visible */
                                            ...
                                            <div className="md:col-span-2 bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-4">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-slate-300 bg-slate-950 p-4 rounded-lg border border-slate-800/40 whitespace-pre-line leading-relaxed">
                                                        {selectedProject.description || "No project description summary defined for this workspace segment instance."}
                                                    </p>
                                                </div>
                                            </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-150">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl h-fit">
                <div className="flex justify-between items-center mb-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
                        ...
                        <span>{editingCredential ? "Modify Asset Key" : "Register New Access Key"}</span>
                    </h4>
                    {editingCredential && (
                        <button type="button" onClick={() => setEditingCredential(null)} className="text-[10px] bg-slate-950 border border-slate-800 text-slate-400 hover:text-white px-2 py-0.5 rounded">Cancel</button>
                    )}
                </div>
                <form 
                    ...
                    onSubmit={async (e) => {
                        ...
                        // 🛠️ CORRECT PAYLOAD ROUTING MATRIX SCHEMA MATCHING:
                        const payload = { 
                            project_id: selectedProject.id, 
                            title: form.title.value, 
                            category: form.category.value, 
                            login_url: form.login_url.value, 
                            auth_type: form.auth_type.value, 
                            email: form.email.value, 
                            username: form.username.value, 
                            encrypted_password: form.encrypted_password.value, // ✅ Reads name="plain_password" and sets key to encrypted_password
                            remarks: form.remarks.value, 
                            client_visible: form.client_visible.checked, 
                            expiry_date: form.expiry_date.value || null 
                        };
                        
                        try { 
                            if (editingCredential) { 
                                await credentialAPI.update(editingCredential.id, payload); 
                                setEditingCredential(null); 
                            } else { 
                                await credentialAPI.create(payload); 
                            } 
                            form.reset(); 
                            await refreshProjectSubData(selectedProject.id); 
                            alert("Access parameter registry securely stored.");
                        } catch (err) { 
                ...
                >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="category" defaultValue={editingCredential ? editingCredential.category : "Domain"} className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2 py-2 text-white focus:outline-none">
                                {['Domain', 'Hosting', 'Vercel', 'Supabase', 'Cloudflare', 'Google', 'Facebook', 'Other'].map(cat => <option key={cat} value={cat}>{cat}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-slate-400 mb-1">Auth Type *</label>
                            <select name="auth_type" defaultValue={editingCredential ? editingCredential.auth_type : "Password"} className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2 py-2 text-white focus:outline-none">
                    ...
                    <div>
                        ...
                        <input name="login_url" type="url" defaultValue={editingCredential ? editingCredential.login_url || "" : ""} placeholder="https://cpanel.host.com" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white focus:outline-none" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className="block text-slate-400 mb-1">Email</label><input name="email" type="email" defaultValue={editingCredential ? editingCredential.email || "" : ""} placeholder="admin@gmail.com" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white focus:outline-none" /></div>
                        <div><label className="block text-slate-400 mb-1">Username</label><input name="username" type="text" defaultValue={editingCredential ? editingCredential.username || "" : ""} placeholder="abc_admin" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white focus:outline-none" /></div>
                    </div>
                    <div>
                        <label className="block text-slate-400 mb-1 font-medium text-slate-300">{editingCredential ? "Password (dots will leave unedited)" : "Password *"}</label>
                        <input required={!editingCredential} name="encrypted_password" type="password" defaultValue={editingCredential ? "••••••••••••" : ""} placeholder="••••••••" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 font-mono text-white focus:outline-none" />
                    </div>
                    <div><label className="block text-slate-400 mb-1">Expiry Date</label><input name="expiry_date" type="date" defaultValue={editingCredential ? editingCredential.expiry_date || "" : ""} className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 font-mono text-white focus:outline-none" /></div>
                    <div><label className="block text-slate-400 mb-1">Remarks</label><textarea name="remarks" rows={2} defaultValue={editingCredential ? editingCredential.remarks || "" : ""} className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white resize-none" /></div>
                    <label className="flex items-center space-x-3 p-2 bg-slate-950 border border-slate-800 rounded-lg cursor-pointer select-none">
                        <input type="checkbox" name="client_visible" defaultChecked={editingCredential?.client_visible || false} className="accent-violet-500 w-4 h-4 rounded" />
                        <div><span className="text-white font-bold block text-[11px]">Allow Client Visibility</span></div>
                    </label>
                    <button type="submit" className="w-full py-2.5 rounded-lg font-semibold text-white bg-violet-600 hover:bg-violet-700 transition">Save Entry</button>
                </form>
            </div>
            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-4">
                ...
                {credentials.length === 0 ? <div className="text-center py-12 text-xs font-mono text-slate-500">No records mapped.</div> : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {credentials.map(c => (
                            <div key={c.id} className="p-4 bg-slate-950 border border-slate-800 rounded-xl flex flex-col justify-between space-y-3 hover:border-slate-700 transition">
                                <div>
                                    <div className="flex justify-between items-start">
                                        <div><h4 className="text-sm font-bold text-white">{c.title}</h4><span className="text-[9px] font-mono text-slate-500 block">Class: {c.auth_type}</span></div>
                                        <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-900 text-violet-400 border border-slate-800">{c.category}</span>
                                    </div>
                                    <div className="space-y-1 font-mono text-[11px] text-slate-400 mt-3 bg-slate-900/30 p-2 rounded border border-slate-800/40">
                                        ...
                                        {c.email && <div>Email: <span className="text-slate-200">{c.email}</span></div>}
                                    </div>
                                </div>
    ...
    <Eye className="w-3.5 h-3.5" />
                                    ...
                                    {projectSubTab === 'notes' && (
                                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-150">
                                            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl h-fit">
                                                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Compose Note Fragment</h4>
                                                <form 
                                                    ...
                                                    onSubmit={async (e) => {
                                                        ...
                                                        try { 
                                                            if (editingNote) { await notesAPI.update(editingNote.id, payload); setEditingNote(null); } 
                                                    ...
                                                    className="space-y-3 text-xs"
                                                >
                                                    <input required name="title" defaultValue={editingNote ? editingNote.title : ""} placeholder="Note Title" className="w-full bg-slate-950 border border-slate-800 px-3 py-2 rounded-md text-white" />
                                                    <textarea required name="content" defaultValue={editingNote ? editingNote.content : ""} placeholder="Markdown logs..." rows={6} className="w-full bg-slate-950 border border-slate-800 px-3 py-2 rounded-md font-mono text-white resize-none" />
                                                    <input name="tags" defaultValue={editingNote ? editingNote.tags || "" : ""} placeholder="Tags (comma separated)" className="w-full bg-slate-950 border border-slate-800 px-3 py-2 rounded-md text-white" />
                                                    <label className="flex items-center space-x-3 p-2 bg-slate-950 border border-slate-800 rounded-lg cursor-pointer">
                                                        <input type="checkbox" name="client_visible" defaultChecked={editingNote?.client_visible || false} className="accent-violet-500 w-4 h-4" />
                                                        <div><span className="text-white font-bold block text-[11px]">Expose to Client Dashboard</span></div>
                                                    </label>
                                                    <button type="submit" className="w-full py-2 rounded-lg font-semibold text-white bg-violet-600 hover:bg-violet-700 transition">Save Note</button>
                                                </form>
                                            </div>
                                            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-4">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {notes.map(n => (
                                                        <div key={n.id} className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2 flex flex-col justify-between shadow-md">
                                                            <div>
                                                                ...
                                                                <p className="text-xs font-mono whitespace-pre-line text-slate-300 mt-1">{n.content}</p>
                                                            </div>
                                                            <div className="flex justify-between items-center pt-2">
                                                                <div className="flex gap-1">{n.tags && n.tags.split(',').map(t=><span key={t} className="text-[9px] bg-violet-950/40 text-violet-400 px-1.5 py-0.5 rounded">#{t.trim()}</span>)}</div>
                                                                <button type="button" onClick={() => setEditingNote(n)} className="text-[11px] bg-slate-900 border border-slate-800 px-2 py-1 rounded text-slate-300">Edit</button>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl h-fit">
                                                ...
                                                <form onSubmit={async (e) => {
                                                    e.preventDefault(); const form = e.target;
                                                    const payload = { project_id: selectedProject.id, title: form.title.value, type: form.type.value, expiry_date: form.expiry_date.value, reminder_days: parseInt(form.reminder_days.value) || 30 };
                                                    try { 
                                                        if (editingReminder) { await remindersAPI.update(editingReminder.id, payload); setEditingReminder(null); } 
                                                    ...
                                                    <button type="submit" className="w-full bg-violet-600 py-2 rounded font-semibold text-white">Save Milestone</button>
                                                </form>
                                            </div>
                                            ...
                                            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl h-fit">
                                                ...
                                                <form onSubmit={async (e) => {
                                                    ...
                                                    fReader.onload = async () => {
                                                        try { 
                                                            await filesAPI.create({ project_id: selectedProject.id, name: form.display_name.value || picker.files[0].name, file_type: form.file_type.value, client_visible: form.client_visible.checked, file_data: fReader.result }); 
                                                            form.reset(); 
                                                            await refreshProjectSubData(selectedProject.id); 
                                                            alert("Binary uploaded.");
                                                        } catch(err) { alert(err.message); }
                                                    };
                                                }} className="space-y-3 text-xs">
                                                    ...
                                                    <label className="border border-dashed border-slate-800 p-4 rounded bg-slate-950 block text-center cursor-pointer text-slate-400"><input type="file" name="file_picker" className="hidden" />Upload File Object</label>
                                                    ...
                                                    <button type="submit" className="w-full bg-violet-600 py-2 rounded text-white font-bold">Upload Binary</button>
                                                </form>
                                            </div>
                                            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-3 text-xs font-mono">
                                                ...
                                                {files.map(f => (
                                                    <div key={f.id} className="p-2.5 bg-slate-950 border border-slate-800 rounded-lg flex justify-between items-center">
                                                        <div className="truncate"><span className="text-white font-sans font-bold block truncate">{f.name}</span><span className="text-[9px] text-slate-500 block">Class: {f.file_type}</span></div>
                                                        <div className="flex gap-2 items-center">
                                                            <button type="button" onClick={async () => { await filesAPI.update(f.id, { ...f, client_visible: !f.client_visible }); await refreshProjectSubData(selectedProject.id); }} className={`px-2 py-0.5 rounded text-[10px] border ${f.client_visible ? 'bg-violet-950 border-violet-900 text-violet-400' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>{f.client_visible ? "Unshare" : "Share"}</button>
                    ...
                    {!loading && activeTab === 'renewals' && (
                        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl max-w-5xl mx-auto space-y-4">
                            <div><h3 className="text-md font-bold text-white mb-1">Global Milestone Renewal Schedule</h3></div>
                            <div className="overflow-x-auto border border-slate-800 rounded-lg bg-slate-950 font-mono text-xs">
                                <table className="w-full text-left border-collapse">
                                    <thead><tr className="border-b border-slate-800 bg-slate-900/50 text-[10px] text-slate-400 uppercase tracking-wider"><th className="p-3">Tracked Item Name</th><th className="p-3">Category Type</th><th className="p-3 text-right">Target Expiry Date</th></tr></thead>
                                    <tbody className="divide-y divide-slate-800/60">
                                        {renewals.map(r => (<tr key={r.id} className="hover:bg-slate-900/30 transition"><td className="p-3 font-semibold text-white">{r.title}</td><td className="p-3 text-slate-400">{r.type}</td><td className="p-3 text-right text-rose-400 font-bold">{r.expiry_date}</td></tr>))}
                                    </tbody>
                                </table>
                            </div>
                        </div>
            ...
            {/* PRIVILEGE CONSOLE MODAL */}
            {showShareModal && sharingProject && (
                <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-xl shadow-2xl p-6 relative flex flex-col max-h-[90vh]">
                        ...
                        <div className="mb-4">
                            <span className="text-[10px] font-mono tracking-wider text-violet-400 font-bold block mb-0.5">ASSIGNMENTS ACCESS GATEWAY</span>
                        ...
                        <div className="grid grid-cols-2 gap-2 bg-slate-950 p-1 border border-slate-800 rounded-lg mb-4">
                            <button type="button" onClick={() => setShareTargetType('employees')} className={`py-1.5 text-xs font-semibold rounded-md transition ${shareTargetType === 'employees' ? 'bg-violet-600 text-white' : 'text-slate-400'}`}>Share with Employees</button>
                            <button type="button" onClick={() => setShareTargetType('clients')} className={`py-1.5 text-xs font-semibold rounded-md transition ${shareTargetType === 'clients' ? 'bg-violet-600 text-white shadow-sm' : 'text-slate-400'}`}>Share with Clients Portal</button>
                        </div>
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            try {
                                ...
                                setProjectShares(Array.isArray(freshShares.data) ? freshShares.data : []);
                        ...
                        <div className="mt-4 flex-1 overflow-y-auto space-y-2 border border-slate-800 p-3 bg-slate-950/40 rounded-xl text-xs">
                            ...
                            {!Array.isArray(projectShares) || projectShares.length === 0 ? (
